...
默认接受纯数字标签，也接受 `TREE:数字`、`TREE-数字` 和 `TREE_数字`。当前现场二维码编号固定为 `1` 到 `156`，按以下列范围解释：第 1 列 `1-42`、第 2 列 `43-77`、第 3 列 `78-121`、第 4 列 `122-156`。

左侧扫描器只接受偶数列，右侧扫描器只接受奇数列。左右两侧的列号必须相邻；初始状态按右侧第 1 列、左侧第 2 列处理，后续例如右侧移动到第 3 列而左侧仍在第 2 列时仍然有效。跳变到不相邻列的扫码结果会被丢弃，原有有效结果保留。默认 30 分钟没有新的有效扫码后该值失效并按无树处理。重复编号在 0.3 秒内只计一次。
