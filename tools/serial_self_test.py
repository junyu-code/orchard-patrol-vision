#!/usr/bin/env python3
"""Serial diagnostics focused on the project's OPGPS receive path."""
...
def _find_port_users(port: str):
    """Return same-user processes with an open fd for a Linux serial port."""
...
def _test_opgps_pipeline(
    ...
    # Clear any completed cross-test bytes before the project receiver owns the port.
...
def run_dual_port_test(
    ...
    """Validate a cross-wired pair in both directions and through OPGPS parsing."""
    if count <= 0:
    ...
    with _physical_serial(sender_port, baudrate, timeout) as sender, _physical_serial(
        ...
        print(f"[2/4] 模拟发送端 {sender_port} -> 项目接收端 {receiver_port}")
        sender_to_receiver = _test_cross_direction(
            sender, serial_b, "TX->RX", count, timeout
        )
        print(f"[3/4] 硬件反向校验 {receiver_port} -> {sender_port}")
    ...
    opgps_ok = True
    if verify_opgps:
        print(f"[4/4] 项目 OPGPS 接收链路：{sender_port} -> {receiver_port}")
    ...
    print(f"项目 GPS 接收：{'通过' if opgps_ok else '失败'}")
    passed = (
...
def run_loopback(
    ...
    """Send real project frames and require the same bytes back."""
    if count <= 0:
        raise ValueError("测试次数必须大于 0")
...
def monitor_gps(port: str, baudrate: int, duration: float, min_packets: int) -> int:
    """Listen on a GPS serial port and validate packets through project code."""
    if duration <= 0:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    dual = subparsers.add_parser(
        "dual",
        help="用一个 USB-TTL 模拟发送，验证另一个端口的项目 GPS 接收链路",
    )
    dual.add_argument(
    ...
    dual.add_argument("--skip-opgps", action="store_true", help="只检查双向字节收发")
    return parser
