...
class HidPosReceiverTests(unittest.TestCase):
    ...
    def test_hid_only_mode_never_falls_back_to_telemetry_tree_ids(self):
    ...
    def test_hid_only_mode_uses_only_scanned_side(self):
        left = HidTagReading("left", 214, "214", "AB031246", 1000)
        snapshot = HidTagSnapshot(
            left=HidTagSideSnapshot(left, 0, False, True),
        ...
        self.assertEqual(result["right_tree_id"], 0)
        self.assertEqual(result["source"], "hid_pos")
    ...
    def test_tree_source_switch_on_ignores_telemetry_ids(self):
        telemetry = {
            "current_tree_id": 9,
            "left_tree_id": 9,
            "right_tree_id": 10,
