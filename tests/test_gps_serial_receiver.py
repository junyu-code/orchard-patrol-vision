...
class GpsSerialReceiverTests(unittest.TestCase):
    ...
    def test_recovers_after_implausible_position_jump(self):
        now = [1000]
        receiver = GpsSerialReceiver(
            port="TEST",
            auto_detect=False,
        ...
        now[0] = 3000
        receiver._handle_line(build_line(sequence=3, latitude=25.07143946))

        self.assertAlmostEqual(
            receiver.get_snapshot(now_ms=3000).speed_mps,
            1.0,
            delta=0.02,
        )
